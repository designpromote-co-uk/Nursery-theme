...
/**
 * Add template-sparkle to body classes
*/
function zn_sparkle_body_class( $classes ) {
	$classes[] = 'template-sparkle';
	return $classes;
}
add_filter( 'body_class', 'zn_sparkle_body_class' );
/**
...
function zn_sparkle_footer_credits( $credit ) {
	$credit = sprintf( __( '%1$s Theme by %2$s | %3$s | ', 'nursery' ), 'Nursery', '<a href="http://designpromote.co.uk/" rel="designer">Designpromote.co.uk</a>', '<a href="' . site_url('/wp-login.php') . '" rel="login">Admin</a>' );
	//return $credit;
	echo $credit;
}
//add_filter( 'infinite_scroll_credit', 'shop_footer_credits' );
add_action( 'nursery_credits', 'zn_sparkle_footer_credits' );
...
function zn_truncate( $content, $limit, $break = '.', $more_link_text = null ) {
...
function nursery_front_page_content( $content ) {
	if( 1 != preg_match( '/more.../', $content ) ) :
		...
		$content = zn_truncate( $content, 350, '.', $more_link_text );
